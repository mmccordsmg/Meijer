...
if earnedoffers:
	for offer in sorted(earnedoffers, key = lambda i: i['meijerOfferId']):
		if offer.get("meijerOfferId") in validids:
			...
			results.append({'meijerOfferId': offerid, 'status': 'valid reward', 'description': f'{offer.get("title").strip()} {strip_tags(offer.get("description")).strip()}', 'expirationDate': offer.get("expirationDate")})
...
if coupons:
	for coupon in sorted(coupons, key = lambda i: i['meijerOfferId']):
		if coupon.get("meijerOfferId") in validids:
			...
			results.append({'meijerOfferId': offerid, 'status': 'valid coupon', 'description': f'{coupon.get("title").strip()} {coupon.get("description").strip()}', 'expirationDate': coupon.get("redemptionEndDate")})
